...
void Player::draw(Camera camera, SDL_Renderer *renderer) const {
    if (!Player::DataWasLoaded) {
        Player::Textures = IMG_LoadTexture(renderer, LinkManager::path("data/images/characters/slime.png", true).c_str());
        Player::Death = Mix_LoadWAV(LinkManager::path("data/sounds/water.mp3", true).c_str());
